	...
	SocketCreate: function(url)
	{
		...
		socket.socket.onmessage = function (e) {	
			...
			for (var i = 0; i < e.data.length; ++i) {
				bytes.push(e.data.charCodeAt(i));
			}
			socket.messages.push(bytes);	
		};
...
autoAddDeps(libraryRealtimeWebGL, '$webSocketInstances');
mergeInto(LibraryManager.library, libraryRealtimeWebGL);
